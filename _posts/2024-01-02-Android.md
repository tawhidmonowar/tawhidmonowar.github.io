...
- `by` and getValue/setValue delegates automatically notify Compose to recompose when the state changes.
...
> Be careful not to keep Activity Context for too long - it can cause memory leaks.
{: .prompt-danger }
